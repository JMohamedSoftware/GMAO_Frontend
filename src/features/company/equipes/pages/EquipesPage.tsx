                  ...
                  <div className="flex flex-col gap-1.5">
                    {teamTechs.map(t => (
                      <div key={t.id} className="flex items-center gap-2 px-2 py-1.5 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-800">
                        ...
                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${t.status === "Disponible" ? "bg-emerald-100 text-emerald-600" : t.status === "Occupé" ? "bg-amber-100 text-amber-600" : "bg-slate-100 text-slate-500"}`}>{t.status}</span>
                      </div>
                    ))}
                  </div>
                )}
                  ...
                  {technicians.map(t => {
                    ...
                    return (
                      <button key={t.id} onClick={() => toggleTech(t.id)} className={`flex items-center gap-3 px-3 py-2 rounded-lg border text-left cursor-pointer transition-all ${selected ? "border-emerald-400 bg-emerald-50 dark:bg-emerald-950/20" : "border-slate-200 dark:border-slate-700 hover:border-emerald-400/50"}`}>
                        ...
                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full flex-shrink-0 ${t.status === "Disponible" ? "bg-emerald-100 text-emerald-600" : t.status === "Occupé" ? "bg-amber-100 text-amber-600" : "bg-slate-100 text-slate-500"}`}>{t.status}</span>
